...
use crate::db::{DbPool, DbRow, Value, fetch_rows};
...
pub async fn run(config_path: &str, out_path: &str, dry_run: bool) -> Result<()> {
    ...
    let target_cfg = cfg.databases.target.as_ref()
    ...
    for table in &cfg.tables {
        ...
        let source_rows = fetch_rows(source_pool.inner(), &table.name, &qualified, &table.columns).await?;
        let target_rows = fetch_rows(target_pool.inner(), &table.name, &qualified, &table.columns).await?;

        let DiffResult { inserts, updates, deletes } = diff_tables(
            &table.name,
            &source_rows,
            &target_rows,
            &table.columns,
...
fn diff_tables(
    ...
    let source_map: HashMap<String, &DbRow> = source_rows.iter()
        .map(|r| (r.pk_key(primary_keys), r))
        .collect();
    let target_map: HashMap<String, &DbRow> = target_rows.iter()
        .map(|r| (r.pk_key(primary_keys), r))
        .collect();
    ...
    for target_row in target_rows {
        ...
        match source_map.get(&key) {
            ...
            Some(source_row) => {
                let changed: Vec<(&str, &Value)> = columns.iter()
    ...
    DiffResult { inserts, updates, deletes }
}
